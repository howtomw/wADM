...
t# ?? **Обязательно к прочтению** 
 
-— 
 
### ? **Час пик:** 
?? **С 15:00 до 20:00 по МСК** 
 
### ? **Норма онлайна:** 
- **Будни:** 1 час 30 минут 
- **Выходные:** 2 часа 
...
## ?? **Донат-Рубли и их выдача** 
 
1. **Выдача строго** за: 
 - выполнение **квестов**; 
 - **активную и полезную** работу Администратора/Лидера. 
2. Можно использовать как **приз на мероприятиях** (МП) с **15+ участников**. 
 - **Лимит**: не более **50 Донат-РУБ.** 
3. Каждая выдача **согласовывается** с **ИО** или **Основателем**. 
 
?? **Важно!** Не обещайте никому Донат-РУБ заранее. 
...
### ??????? **Следящие:** 
- **ГС, ЗГС ГОСС:** `/makeleader`, `/offleader` 
- **ГС, ЗГС МАФИИ:** `/makeleader`, `/offleader` 
- **ГС, ЗГС ГЕТТО:** `/makeleader`, `/offleader`, `/gzcolor`, `/ghetto`, `/warn`, `/stopcapt` 
- **ГС, ЗГС ХЕЛПЕРОВ:** `/makehelper`, `/offhelper` 
 
? *Помощникам ГС команды не выдаются.* 
 
-— 
 
### ??????? **Ведущая администрация:** 
- **ЗГА:** `/avig`, `/aunvig`, `/warn`, `/ban` 
- **ГА:** `/avig`, `/aunvig`, `/warn`, `/ban` 
- **Руководитель:** `/avig`, `/aunvig`, `/warn`, `/ban` 
 
-— 
 
### ?????> **Красная администрация:** 
- **ЗКС:** `/makeleader`, `/offleader`, `/banip`, `/ban`, `/warn`, `/avig`, `/aunvig`, `/gzcolor`, `/ghetto`, `/astats`, `/getip`, `/pgetip` 
- **КС:** `/makeleader`, `/offleader`, `/banip`, `/ban`, `/warn`, `/avig`, `/aunvig`, `/gzcolor`, `/ghetto`, `/astats`, `/getip`, `/pgetip` 
 
-— 
 
### ???? **Руководящая администрация (FD):** 
- **П.О:** `/offadmin`, `/astats`, `/getip`, `/setstat`, `/pgetip`, `/agetipreg`, `/namehistory` 
- **З.О:** `/unbanby`, `/offadmin`, `/astats`, `/prefix`, `/getip`, `/setstat`, `/pgetip`, `/agetipreg`, `/logs`, `/namehistory`, `/setcmd` 
 
-— 
 
## ?? **Дополнительные правила** 
 
### ?? **Восстановление на пост администратора:** 
- **9–13 уровень:** минус **2 уровня**. 
- **7–8 уровень:** минус **1 уровень**. 
- **4–6 уровень:** восстановление **без потерь**. 
 
### ?? **Выдача префиксов:** 
- **Главный следящий:** `MAFIA / GOSS / GHETTO / HELPERS` 
- **Заместитель ГС:** `MAFIA / GOSS / GHETTO / HELPERS` 
- **Помощник ГС:** `MAFIA / GOSS / GHETTO / HELPERS` 
 
-— 
 
?? **Соблюдайте правила и удачной работы!** ??

]]
script_name("wADM")
script_description('This is a Cross-platform Lua script helper for Admins-Team')
script_author("howtomw")
script_version("1.4")
...
local configDirectory = getWorkingDirectory():gsub('\\','/') .. "/wADM"
local path_helper = getWorkingDirectory():gsub('\\','/') .. "/wADM.lua"
...
function load_settings()
    ...
    if not doesFileExist(path_settings) then
		...
		print('[wADM] Файл с настройками не найден, использую стандартные настройки!')
    else
        ...
        if file then
			...
			if #contents == 0 then
				settings = default_settings
				print('[wADM] Не удалось открыть файл с настройками, использую стандартные настройки!')
			else
				local result, loaded = pcall(decodeJson, contents)
				if result then
					settings = loaded
					print('[wADM] Настройки успешно загружены!')
					...
					if settings.general.version ~= thisScript().version then
						print('[wADM] Новая версия, сброс настроек!')
						settings = default_settings
						save_settings()
						reload_script = true
						thisScript():reload()
...
function save_settings()
    ...
    if file then
        ...
        file:close()
		print('[wADM] Настройки сохранены!')
        return result
    else
        print('[wADM] Не удалось сохранить настройки хелпера, ошибка: ', errstr)
        return false
    end
end
load_settings()
...
local notes = {
	note = {
		{ note_name = 'Лучший тулс?)', note_text = 'Ну лучший же?)'},
	}
}
...
function load_notes()
	if doesFileExist(path_notes) then
        ...
        if file then
            ...
            file:close()
			if #contents == 0 then
				print('[wADM] Не удалось открыть файл с заметками!')
				print('[wADM] Причина: этот файл пустой')
			else
				local result, loaded = pcall(decodeJson, contents)
				if result then
					notes = loaded
					print('[wADM] Заметки инициализированы!')
				else
					print('[wADM] Не удалось открыть файл с заметками!')
					print('[wADM] Причина: Не удалось декодировать json (ошибка в файле)')
				end
			end
        else
			print('[wADM] Не удалось открыть файл с заметками!')
		...
		print('[wADM] Не удалось открыть файл с заметками!')
		print('[wADM] Причина: этого файла нету в папке '..configDirectory)
	end
end
function save_notes()
    ...
    if file then
        ...
        file:write(result and encoded or "")
        file:close()
		print('[wADM] Заметки сохранены!')
        return result
    else
        print('[wADM] Не удалось сохранить заметки, ошибка: ', errstr)
        return false
    end
end
load_notes()
...
function load_smart_uk()
	if doesFileExist(path_uk) then
        ...
        if file then
            ...
            file:close()
			if #contents == 0 then
				print('[wADM] Не удалось открыть файл с умным розыском!')
				print('[wADM] Причина: этот файл пустой')
			else
				local result, loaded = pcall(decodeJson, contents)
				if result then
					smart_uk = loaded
					print('[wADM] Умный розыск инициализирован!')
				else
					print('[wADM] Не удалось открыть файл с умным розыском!')
					print('[wADM] Причина: Не удалось декодировать json (ошибка в файле)')
				end
...
function save_smart_uk()
    ...
    if file then
        ...
        file:write(result and encoded or "")
        file:close()
		print('[wADM] Умный розыск сохранён!')
        return result
    else
        print('[wADM] Не удалось сохранить умный розыск, ошибка: ', errstr)
        return false
    end
end
load_smart_uk()
...
function load_smart_pdd()
	if doesFileExist(path_pdd) then
        ...
        if file then
            ...
            file:close()
			if #contents == 0 then
				print('[wADM] Не удалось открыть файл с умным штрафом!')
				print('[wADM] Причина: этот файл пустой')
			else
				local result, loaded = pcall(decodeJson, contents)
				if result then
					smart_pdd = loaded
					print('[wADM] Умный штраф инициализирован!')
				else
					print('[wADM] Не удалось открыть файл с умным штрафом!')
					print('[wADM] Причина: Не удалось декодировать json (ошибка в файле)')
				end
...
function save_smart_pdd()
    ...
    if file then
        ...
        file:write(result and encoded or "")
        file:close()
		print('[wADM] Умные штрафы сохранены!')
        return result
    else
        print('[wADM] Не удалось сохранить умные штрафы, ошибка: ', errstr)
        return false
    end
end
load_smart_pdd()
...
local commands = {
	commands = {
        { cmd = 'wgo' , description = 'Начинаю работать по жалобе' ,  text = '/pm {arg_id} Здравствуйте, уважаемый игрок!&/pm {arg_id} Начинаю работать по вашей жалобе!' , arg = '{arg_id}' , enable = true , waiting = '1.200'},
	},
}
...
function load_commands()
	if doesFileExist(path_commands) then
        ...
        if file then
            ...
            file:close()
			if #contents == 0 then
				print('[wADM] Не удалось открыть файл с командами!')
				print('[wADM] Причина: этот файл пустой')
			else
				local result, loaded = pcall(decodeJson, contents)
				if result then
					commands = loaded
					print('[wADM] Все команды инициализирован!')
				else
					print('[wADM] Не удалось открыть файл с командами!')
					print('[wADM] Причина: Не удалось декодировать json (ошибка в файле)')
				end
		...
		print('[wADM] Не удалось открыть файл с командами!')
		print('[wADM] Причина: этого файла нету в папке '..configDirectory)
		print('[wADM] Инициализация стандартных команд...')
		save_commands()
		load_commands()
	end
end
function save_commands()
    ...
    if file then
        ...
        file:write(result and encoded or "")
        file:close()
		print('[wADM] Ваши команды сохранены!')
        return result
    else
        print('[wADM] Не удалось сохранить команды хелпера, ошибка: ', errstr)
        return false
    end
end
load_commands()
...
function load_arzvehicles()
	if doesFileExist(path_arzvehicles) then
        ...
        if file then
            ...
            file:close()
			if #contents == 0 then
				print('[wADM] Не удалось открыть файл с моделями каров аризоны!')
				print('[wADM] Причина: этот файл пустой')
			else
				local result, loaded = pcall(decodeJson, contents)
				if result then
					arzvehicles = loaded
					print('[wADM] Модели кастом каров аризоны инициализированы!')
				else
					print('[wADM] Не удалось открыть файл с моделями каров аризоны!')
					print('[wADM] Причина: Не удалось декодировать json (ошибка в файле)')
				end
...
local tagReplacements = {
	...
	get_storecar_model = function ()
		...
		if closest_car then
			...
			sampAddChatMessage("[wADM] {ffffff}Не удалось получить модель ближайшего транспорта с водителем!", 0x009EFF)
			return ''
		end
	end,
		...
		if isCharInAnyCar(PLAYER_PED) then
			...
			if success and passengers and tonumber(passengers) > 0 then
				...
				if #my_passengers ~= 0 then
					...
					sampAddChatMessage('[wADM] В вашем авто нету ваших напарников12345678!', -1)
					return 'Нету'
				end
			else
				return 'Нету'
			...
			--sampAddChatMessage('[wADM] Вы не находитесь в авто, невозможно получить ваших напарников!', -1)
			return 'Нету'
		end
	end
...
local message_color = 0x0b9c5f
local message_color_hex = '{0b9c5f}'
...
function welcome_message()
	if not sampIsLocalPlayerSpawned() then 
		sampAddChatMessage('[wADM] {ffffff}Инициализация wADM прошла успешно!',message_color)
		sampAddChatMessage('[wADM] {ffffff}Для полной загрузки нужно заспавниться (войдите на сервер)',message_color)
		repeat wait(0) until sampIsLocalPlayerSpawned()
	end
	sampAddChatMessage('[wADM] {ffffff}Загрузка прошла успешно!', message_color)
	show_cef_notify('info', 'wADM', "Загрузка прошла успешно!", 3000)
	if isMonetLoader() or settings.general.bind_mainmenu == nil or not settings.general.use_binds then	
		sampAddChatMessage('[wADM] {ffffff}Чтобы открыть меню введите команду ' .. message_color_hex .. '/wadm', message_color)
	elseif hotkey_no_errors and settings.general.bind_mainmenu and settings.general.use_binds then
		sampAddChatMessage('[wADM] {ffffff}Чтобы открыть меню нажмите ' .. message_color_hex .. getNameKeysFrom(settings.general.bind_mainmenu) .. ' {ffffff}или введите команду ' .. message_color_hex .. '/wadm', message_color)
	else
		sampAddChatMessage('[wADM] {ffffff}Чтобы открыть меню введите команду ' .. message_color_hex .. '/wadm', message_color)
	end
end
		...
		if not isActiveCommand then
			...
			if cmd_arg == '{arg}' then
				if arg and arg ~= '' then
					...
					sampAddChatMessage('[wADM] {ffffff}Используйте ' .. message_color_hex .. '/' .. chat_cmd .. ' [аргумент]', message_color)
					play_error_sound()
				end
			elseif cmd_arg == '{arg_id}' then
				if isParamSampID(arg) then
					...
					sampAddChatMessage('[wADM] {ffffff}Используйте ' .. message_color_hex .. '/' .. chat_cmd .. ' [ID игрока]', message_color)
					play_error_sound()
				end
			elseif cmd_arg == '{arg_id} {arg2}' then
				if arg and arg ~= '' then
					...
					if isParamSampID(arg_id) and arg2 then
						...
						sampAddChatMessage('[wADM] {ffffff}Используйте ' .. message_color_hex .. '/' .. chat_cmd .. ' [ID игрока] [аргумент]', message_color)
						play_error_sound()
					end
				else
					sampAddChatMessage('[wADM] {ffffff}Используйте ' .. message_color_hex .. '/' .. chat_cmd .. ' [ID игрока] [аргумент]', message_color)
            ...
            elseif cmd_arg == '{arg_id} {arg2} {arg3}' then
				if arg and arg ~= '' then
					...
					if isParamSampID(arg_id) and arg2 and arg3 then
						...
						sampAddChatMessage('[wADM] {ffffff}Используйте ' .. message_color_hex .. '/' .. chat_cmd .. ' [ID игрока] [аргумент] [аргумент]', message_color)
						play_error_sound()
					end
				else
					sampAddChatMessage('[wADM] {ffffff}Используйте ' .. message_color_hex .. '/' .. chat_cmd .. ' [ID игрока] [аргумент] [аргумент]', message_color)
				...
				lua_thread.create(function()
					...
					if modifiedText:find('&.+&') then
						if isMonetLoader() and settings.general.mobile_stop_button then
							sampAddChatMessage('[wADM] {ffffff}Чтобы остановить отыгровку команды используйте ' .. message_color_hex .. '/stop {ffffff}или нажмите кнопку внизу экрана', message_color)
						...
						elseif not isMonetLoader() and hotkey_no_errors and settings.general.bind_command_stop and settings.general.use_binds then
							sampAddChatMessage('[wADM] {ffffff}Чтобы остановить отыгровку команды используйте ' .. message_color_hex .. '/stop {ffffff}или нажмите ' .. message_color_hex .. getNameKeysFrom(settings.general.bind_command_stop), message_color)
						else
							sampAddChatMessage('[wADM] {ffffff}Чтобы остановить отыгровку команды используйте ' .. message_color_hex .. '/stop', message_color)
						end
					end
					local lines = {}
					...
					for line_index, line in ipairs(lines) do
						if command_stop then 
							...
							sampAddChatMessage('[wADM] {ffffff}Отыгровка команды /' .. chat_cmd .. " успешно остановлена!", message_color) 
							return 
						end
						...
						elseif line == "{pause}" then
							sampAddChatMessage('[wADM] {ffffff}Команда /' .. chat_cmd .. ' поставлена на паузу!', message_color)
							command_pause = true
							...
							if not command_stop then
								sampAddChatMessage('[wADM] {ffffff}Продолжаю отыгровку команды /' .. chat_cmd, message_color)	
							end					
						else
							if line_index ~= 1 then wait(cmd_waiting * 1000) end
							sampSendChat(line)
							if debug_mode then sampAddChatMessage('[wADM DEBUG] {ffffff}SEND: ' .. line, message_color) end
						end
					end
					isActiveCommand = false
					if isMonetLoader() and settings.general.mobile_stop_button then
			...
			sampAddChatMessage('[wADM] {ffffff}Дождитесь завершения отыгровки предыдущей команды!', message_color)
			play_error_sound()
		end
	end)
end
function find_and_use_command(cmd, cmd_arg)
	...
	if not check then
		sampAddChatMessage('[wADM] {ffffff}Ошибка, не могу найти бинд для выполнения этой команды!', message_color)
		play_error_sound()
		return
	end
end
function initialize_commands()
	sampRegisterChatCommand("wadm", function() MainWindow[0] = not MainWindow[0]  end) -- reg cmd
	sampRegisterChatCommand("wfast", show_fast_menu) -- reg cmd 2
	sampRegisterChatCommand("stop", function() 
		if isActiveCommand then 
			command_stop = true 
		else 
			sampAddChatMessage('[wADM] {ffffff}В данный момент нету никакой активной команды!', message_color) 
			play_error_sound()
		end
	end)
		...
		if not isActiveCommand then
			if isParamSampID(arg) then
				if #smart_uk ~= 0 then
					...
					sampAddChatMessage('[wADM] {ffffff}Сначало загрузите/отредактируйте умный розыск в /wadm', message_color)
					play_error_sound()
				end
			else
				sampAddChatMessage('[wADM] {ffffff}Используйте ' .. message_color_hex .. '/sum [ID игрока]', message_color)
				play_error_sound()
			end	
		else
			sampAddChatMessage('[wADM] {ffffff}Дождитесь завершения отыгровки предыдущей команды!', message_color)
		...
		if not isActiveCommand then
			if isParamSampID(arg) then
				if #smart_pdd ~= 0 then
					...
					sampAddChatMessage('[wADM] {ffffff}Сначало загрузите/отредактируйте умные штрафы в /wadm', message_color)
					play_error_sound()
				end
			else
				sampAddChatMessage('[wADM] {ffffff}Используйте ' .. message_color_hex .. '/tsm [ID игрока]', message_color)
				play_error_sound()
			end	
		else
			sampAddChatMessage('[wADM] {ffffff}Дождитесь завершения отыгровки предыдущей команды!', message_color)
	...
	--[[sampRegisterChatCommand("sob", function(arg)
		if not isActiveCommand then
			if isParamSampID(arg) then
				...
				sampAddChatMessage('[wADM] {ffffff}Используйте ' .. message_color_hex .. '/sob [ID игрока]', message_color)
				play_error_sound()
			end	
		else
			sampAddChatMessage('[wADM] {ffffff}Дождитесь завершения отыгровки предыдущей команды!', message_color)
...
--[[	sampRegisterChatCommand("nonoFGzkT56m2z__!", function(arg)
		if WantedWindow[0] then
			WantedWindow[0] = false
			sampAddChatMessage('[wADM] {ffffff}Меню списка преступников закрыто!', message_color)
		elseif not isActiveCommand then
			lua_thread.create(function()
				sampAddChatMessage('[wADM] {ffffff}Начинаю сканирование /wanted [1-7], ожидайте...', message_color)
				show_cef_notify('info', 'wADM', "Сканирование /wanted...", 2500)
				wanted_new = {}
				check_wanted = true
				for i = 7, 1, -1 do
				...
				if #wanted_new == 0 then
					sampAddChatMessage('[wADM] {ffffff}Сейчас на сервере нету игроков с розыском!', message_color)
				else
					sampAddChatMessage('[wADM] {ffffff}Сканирование успешно заверешено', message_color)
					wanted = wanted_new
					WantedWindow[0] = true
				end
			end)
	...
	sampRegisterChatCommand("debug_howtomw!", function() debug_mode = not debug_mode end)
	--sampRegisterChatCommand("afind", function() sampAddChatMessage('[wADM] {ffffff}Данная функция доступна только в платной версии 2.0 (покупать у MTG MODS)', message_color) end)
			...
			if sampGetPlayerColor(select(2, sampGetPlayerIdByCharHandle(PLAYER_PED))) == 23486046 then
				lua_thread.create(function()
					...
					if command_stop then 
						...
						sampAddChatMessage('[wADM] {ffffff}Отыгровка команды /mask успешно остановлена!', message_color) 
						return
					end
			...
			if not isActiveCommand then
				lua_thread.create(function()
					...
					if command_stop then 
						...
						sampAddChatMessage('[wADM] {ffffff}Отыгровка команды /spcar успешно остановлена!', message_color) 
						return
					end
...
function play_error_sound()
	...
	show_cef_notify('error', 'wADM', "Произошла ошибка!", 1500)
end
	...
	if isParamSampID(id) then 
		...
		if isMonetLoader() or settings.general.bind_fastmenu == nil then
			if not FastMenuPlayers[0] then
				sampAddChatMessage('[wADM] {ffffff}Используйте ' .. message_color_hex .. '/wfast [ID]', message_color)
			end
		elseif settings.general.bind_fastmenu and settings.general.use_binds and hotkey_no_errors then
			sampAddChatMessage('[wADM] {ffffff}Используйте ' .. message_color_hex .. '/wfast [ID] {ffffff}или наведитесь на игрока через ' .. message_color_hex .. 'ПКМ + ' .. getNameKeysFrom(settings.general.bind_fastmenu), message_color) 
		else
			sampAddChatMessage('[wADM] {ffffff}Используйте ' .. message_color_hex .. '/wfast [ID]', message_color)
		end 
		play_error_sound()
...
function show_leader_fast_menu(id)
	if isParamSampID(id) then
		...
		if isMonetLoader() or settings.general.bind_leader_fastmenu == nil then
			sampAddChatMessage('[wADM] {ffffff}Используйте ' .. message_color_hex .. '/jlm [ID]', message_color)
		elseif settings.general.bind_leader_fastmenu and settings.general.use_binds and hotkey_no_errors then
			sampAddChatMessage('[wADM] {ffffff}Используйте ' .. message_color_hex .. '/jlm [ID] {ffffff}или наведитесь на игрока через ' .. message_color_hex .. 'ПКМ + ' .. getNameKeysFrom(settings.general.bind_leader_fastmenu), message_color) 
		else
			sampAddChatMessage('[wADM] {ffffff}Используйте ' .. message_color_hex .. '/jlm [ID]', message_color)
		end 
		play_error_sound()
		...
		if #arzvehicles ~= 0 then
			for _, vehicle in ipairs(arzvehicles) do
				if vehicle.model_id == id then
					--sampAddChatMessage("[wADM] {ffffff}Самый ближайший транспорта к вам это " .. vehicle.name ..  " [ID " .. id .. "].", message_color)
					return vehicle.name
				end
			end
		else
			sampAddChatMessage('[wADM] {ffffff}Не удалось получить модель транспорта с ID ' .. id .. "! Причина: ошибка инициализации VehiclesArizona.json", message_color)
			load_arzvehicles()
			return ''
		end
	else
		sampAddChatMessage('[wADM] {ffffff}Не удалось получить модель транспорта с ID ' .. id .. "! Причина: отсуствует файл VehiclesArizona.json", message_color)
		sampAddChatMessage('[wADM] {ffffff}Пытаюсь скачать файл VehiclesArizona.json в папку ' .. path_arzvehicles, message_color)
...
function check_update()
	print('[wADM] Начинаю проверку на наличие обновлений...')
	sampAddChatMessage('[wADM] {ffffff}Начинаю проверку на наличие обновлений...', message_color)
	local path = configDirectory .. "/Update_Info.json"
	os.remove(path)
	local url = 'https://github.com/howtomw/wADM/blob/main/Update_Info.json'
	if isMonetLoader() then
		downloadToFile(url, path, function(type, pos, total_size)
			if type == "finished" then
				...
				if updateInfo then
					...
					print("[wADM] Текущая установленная версия:", thisScript().version)
					print("[wADM] Текущая версия в облаке:", uVer)
					if thisScript().version ~= uVer then
						print('[wADM] Доступно обновление!')
						sampAddChatMessage('[wADM] {ffffff}Доступно обновление!', message_color)
						...
						print('[wADM] Обновление не нужно!')
						sampAddChatMessage('[wADM] {ffffff}Обновление не нужно, у вас актуальная версия!', message_color)
					end
				end
			end
		end)
	...
	function readJsonFile(filePath)
		if not doesFileExist(filePath) then
			print("[wADM] Ошибка: Файл " .. filePath .. " не существует")
			return nil
		end
		...
		if not jsonData then
			print("[wADM] Ошибка: Неверный формат JSON в файле " .. filePath)
			return nil
		end
		return jsonData
	end
...
function downloadFileFromUrlToPath(url, path)
	print('[wADM] Начинаю скачивание файла в ' .. path)
	if isMonetLoader() then
		downloadToFile(url, path, function(type, pos, total_size)
			...
			elseif type == "finished" then
				if download_helper then
					sampAddChatMessage('[wADM] {ffffff}Загрузка новой версии хелпера завершена успешно! Перезагрузка..',  message_color)
					reload_script = true
					thisScript():unload()
				elseif download_smartuk then
					sampAddChatMessage('[wADM] {ffffff}Загрузка умной выдачи розыска для сервера ' .. getARZServerName(getARZServerNumber()) .. '[' .. getARZServerNumber() ..  '] завершена успешно!',  message_color)
					download_smartuk = false
					load_smart_uk()
				elseif download_smartpdd then
					sampAddChatMessage('[wADM] {ffffff}Загрузка умной выдачи штрафов для сервера ' .. getARZServerName(getARZServerNumber()) .. '[' .. getARZServerNumber() ..  '] завершена успешно!',  message_color)
					download_smartpdd = false
					load_smart_pdd()
				elseif download_arzvehicles then
					sampAddChatMessage('[wADM] {ffffff}Загрузка списка моделей кастом каров аризоны заверешена успешно!',  message_color)
					download_arzvehicles = false
					load_arzvehicles()
				end
			elseif type == "error" then
				sampAddChatMessage('[wADM] {ffffff}Ошибка загрузки: ' .. pos,  message_color)
			end
		end)
	else
		downloadUrlToFile(url, path, function(id, status)
...
function sampev.onShowTextDraw(id, data)
	if data.text:find('~n~~n~~n~~n~~n~~n~~n~~n~~w~Style: ~r~Sport!') then
		sampAddChatMessage('[wADM] {ffffff}Активирован режим езды Sport!', message_color)
		return false
	end
	if data.text:find('~n~~n~~n~~n~~n~~n~~n~~n~~w~Style: ~g~Comfort!') then
		sampAddChatMessage('[wADM] {ffffff}Активирован режим езды Comfort!', message_color)
		return false
	end
end
...
function sampev.onSendTakeDamage(playerId,damage,weapon)
	if playerId ~= 65535 then
		...
		if isParamSampID(playerId) and playerId1 ~= playerId2 and tonumber(playerId) ~= 0 and weapon then
			...
			if weapon_name then
				sampAddChatMessage('[wADM] {ffffff}Игрок ' .. sampGetPlayerNickname(playerId) .. '[' .. playerId .. '] напал на вас используя ' .. weapon_name .. ', ситуационный код изменён на CODE 0.', message_color)
...
function sampev.onServerMessage(color,text)
	...
	if (settings.general.auto_uval and tonumber(settings.player_info.fraction_rank_number) >= 9) then
		...
		if text:find("(.+) заглушил%(а%) игрока (.+) на 1 минут. Причина: %[AutoUval%] Ожидайте...") and auto_uval_checker then
			...
			lua_thread.create(function ()
				wait(50)
				if Name == MyName then
					sampAddChatMessage('[wADM] {ffffff}Увольняю игрока ' .. sampGetPlayerNickname(PlayerID) .. '!', message_color)
					auto_uval_checker = false
					temp = PlayerID .. ' ПСЖ'
					find_and_use_command("/uninvite {arg_id} {arg2}", temp)
				else
					sampAddChatMessage('[wADM] {ffffff}Другой заместитель/лидер уже увольняет игрока ' .. sampGetPlayerNickname(PlayerID) .. '!', message_color)
					auto_uval_checker = false
				end
			end)
		end
	...
	if tonumber(settings.player_info.fraction_rank_number) >= 5 then
		if text:find("%[(.-)%] (.-) (.-)%[(.-)%]: (.+)") and color == 766526463 then -- /f /fb или /r /rb без тэга 
			...
			if message:find('Прошу обьявить в розыск (%d) степени дело N(%d+)%. Причина%: (.+)') then
				...
				sampAddChatMessage('[wADM] {ffffff}Используйте /givefsu ' .. playerID .. ' чтобы выдать розыск по запросу офицера!', message_color)
			end
		elseif text:find("%[(.-)%] %[(.-)%] (.+) (.-)%[(.-)%]: (.+)") and color == 766526463 then -- /r или /f с тэгом
			local tag, tag2, rank, name, playerID, message = string.match(text, "%[(.-)%] %[(.-)%] (.+) (.-)%[(.-)%]: (.+)")
			local lvl, id, reason = message:match('Прошу обьявить в розыск (%d) степени дело N(%d+)%. Причина%: (.+)')
	...
	if (text:find("У (.+) отсутствует трудовая книжка. Вы можете выдать ему книжку с помощью команды /givewbook") and tonumber(settings.player_info.fraction_rank_number) >= 9) then
		...
		sampAddChatMessage('[wADM] {ffffff}У игрока ' .. nick .. ' нету трудовой книжки, выдайте её используя ' .. message_color_hex .. cmd .. ' ' .. sampGetPlayerIdByNickname(nick), message_color)
		return false
	end
	if (settings.general.auto_mask) then
		if text:find('Время действия маски истекло, вам пришлось ее выбросить.') then
			sampAddChatMessage('[wADM] {ffffff}Время действия маски истекло, автоматически надеваю новую', message_color)
			sampSendChat("/mask")
			return false
		elseif (text:find('Время действия маски (%d+) минут, после исхода времени ее придётся выбросить.')) then
			...
			sampAddChatMessage('[wADM] {ffffff}Время действия маски ' .. min .. ' минут, после исхода времени автоматически надеву новую!', message_color)
			return false
		end
	end 
	...
	if text:find("Номера телефонов государственных служб:") then
		sampAddChatMessage('[wADM] {ffffff}Номера телефонов государственных служб:', message_color)
		sampAddChatMessage('[wADM] {ffffff}111 Баланс | 60 Время | 911 МЮ | 912 МЗ | 913 Такси | 914 Мехи | 8828 Банк | 997 Дома', message_color)
		return false
	end
	if text:find ('{FFFFFF}Время действия маски 20 минут, после исхода времени ее придётся выбросить.') then
		sampAddChatMessage('[wADM] {ffffff}Время действия маски 20 минут, после исхода времени автоматически надену новую', message_color)
		return false
	end
	if text:find ('Время действия маски истекло, вам пришлось ее выбросить.') then
		sampAddChatMessage('[wADM] {ffffff}Время действия маски истекло! Автоматически надеваю новую', message_color)
	...
	if text:find("Вы успешно надели маску") then
		maska = true
		sampAddChatMessage('[wADM] {ffffff}Вы надели маску', message_color)
		return false
	end
	...
	if text:find("Вы успешно выкинули маску") or text:find("Вы сняли маску") then
		sampAddChatMessage('[wADM] {ffffff}Вы сняли маску!', message_color)
		return false
	end
...
function sampev.onShowDialog(dialogid, style, title, button1, button2, text)
	...
	if title:find('Основная статистика') and check_stats then -- получение статистики
		if text:find("{FFFFFF}Имя: {B83434}%[(.-)]") then
			...
			sampAddChatMessage('[wADM] {ffffff}Ваше Имя и Фамилия обнаружены, вы - ' .. settings.player_info.name_surname, message_color)
		end
		if text:find("{FFFFFF}Пол: {B83434}%[(.-)]") then
			settings.player_info.sex = text:match("{FFFFFF}Пол: {B83434}%[(.-)]")
			sampAddChatMessage('[wADM] {ffffff}Ваш пол обнаружен, вы - ' .. settings.player_info.sex, message_color)
		end
		if text:find("{FFFFFF}Организация: {B83434}%[(.-)]") then
			...
			if settings.player_info.fraction == 'Не имеется' then
				sampAddChatMessage('[wADM] {ffffff}Вы не состоите в организации!',message_color)
				settings.player_info.fraction_tag = "Неизвестно"
			else
				sampAddChatMessage('[wADM] {ffffff}Ваша организация обнаружена, это: '..settings.player_info.fraction, message_color)
				...
				sampAddChatMessage('[wADM] {ffffff}Вашей организации присвоен тег '..settings.player_info.fraction_tag .. ". Но вы можете изменить его.", message_color)
			end
		end
		if text:find("{FFFFFF}Должность: {B83434}(.+)%((%d+)%)") then
			...
			sampAddChatMessage('[wADM] {ffffff}Ваша должность обнаружена, это: '..settings.player_info.fraction_rank.." ("..settings.player_info.fraction_rank_number..")", message_color)
			if tonumber(settings.player_info.fraction_rank_number) >= 9 then
			...
			sampAddChatMessage('[wADM] {ffffff}Произошла ошибка, не могу получить ваш ранг!',message_color)
		end
		save_settings()
	...
	if members_check and title:find('(.+)%(В сети: (%d+)%)') then -- мемберс 
		...
		elseif #members_new ~= 0 then
			...
			sampSendDialogResponse(dialogid, 0, 0, 0)
			sampAddChatMessage('[wADM]{ffffff} Список сотрудников пуст!', message_color)
			members_check = false
        end
        return false
    end
	...
	if title:find('Дата и время убийства') then
		...
		if debug_mode then sampAddChatMessage('[wADM] Автозаполнение данных: ' .. data , message_color) end
	...
	if title:find('Орудие убийства') then
		if text:find("Неизвестно") then
			...
			if debug_mode then sampAddChatMessage('[wADM] {ffffff}Автозаполнение данных: ' .. data , message_color) end
			sampSendDialogResponse(dialogid, 1, 0, data)
		end
		return false
	end
	...
	if isMonetLoader() then
		...
		if id == 220 then
			...
			if raknetBitStreamReadInt8(bs) == 17 then
				...
				if cmd2:find('Основная статистика') and check_stats then -- /wfaste
					sampAddChatMessage('[wADM] {ffffff}Ошибка, не могу получить данные из нового CEF диалога!', message_color)
					sampAddChatMessage('[wADM] {ffffff}Включите старый (класичесский) вид диалогов в /settings - Кастомизация интерфейса', message_color)
...
imgui.OnFrame(
    ...
    function(player)
		...
		imgui.Begin(fa.CLOUD .. " wADM ##main", MainWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize )
		if imgui.BeginTabBar('пон') then	
			if imgui.BeginTabItem(fa.HOUSE..u8' Главное меню') then
				if imgui.BeginChild('##1', imgui.ImVec2(589 * MONET_DPI_SCALE, 171 * MONET_DPI_SCALE), true) then
					imgui.CenterText(fa.USER_NURSE .. u8' Информация про Администратора')
					imgui.Separator()
					imgui.Columns(3)
					imgui.CenterColumnText(u8"Имя и Фамилия:")
					imgui.SameLine(nil, 5) imgui.TextDisabled("[?]")
					if imgui.IsItemHovered() then
						imgui.SetTooltip(u8"Вывод информации с Вашим ник-неймом")
					end
					...
					imgui.SameLine(nil, 5) imgui.TextDisabled("[?]")
					if imgui.IsItemHovered() then
						imgui.SetTooltip(u8"Для правильных сообщений в чате [Женский/Мужской пол]")
					end
					imgui.NextColumn()
					...
					imgui.SameLine(nil, 5) imgui.TextDisabled("[?]")
					if imgui.IsItemHovered() then
						imgui.SetTooltip(u8"Ну это акцент, че сказать")
					end
					imgui.NextColumn()
				...
				if imgui.BeginChild('##2', imgui.ImVec2(589 * MONET_DPI_SCALE, 53 * MONET_DPI_SCALE), true) then
					imgui.CenterText(fa.ROBOT .. u8' Помощничек')
					imgui.SameLine(nil, 5) imgui.TextDisabled("[?]")
					if imgui.IsItemHovered() then
						imgui.SetTooltip(u8"Ваш незаменимый помощник для автоматизации некоторых действий")
					end
					imgui.Separator()
					imgui.Columns(2)
					...
					imgui.SetColumnWidth(-1, 100 * MONET_DPI_SCALE)
					imgui.Columns(1)
					imgui.CenterColumnText(u8("/wfast [ID] - Быстрое взаимодействие с игроком"))
					imgui.SameLine(nil, 5) imgui.TextDisabled("[?]")
					if imgui.IsItemHovered() then
						imgui.SetTooltip(u8"Реализовано быстрое взаимодействие с игроком")
					end
				imgui.EndChild()
				end
				if imgui.BeginChild('##3', imgui.ImVec2(589 * MONET_DPI_SCALE, 127 * MONET_DPI_SCALE), true) then
					...
					imgui.SameLine(nil, 5) imgui.TextDisabled("[?]")
					if imgui.IsItemHovered() then
						imgui.SetTooltip(u8"Отображение на экране меню с информацией")
					end
					imgui.SetColumnWidth(-1, 230 * MONET_DPI_SCALE)
					imgui.NextColumn()
			...
			if imgui.BeginTabItem(fa.RECTANGLE_LIST..u8' Команды') then 
				if imgui.BeginTabBar('Tabs2') then
					if imgui.BeginTabItem(fa.BARS..u8' Общие команды ') then 
						if imgui.BeginChild('##1', imgui.ImVec2(589 * MONET_DPI_SCALE, 303 * MONET_DPI_SCALE), true) then
							...
							imgui.Separator()
							imgui.Columns(3)
							imgui.CenterColumnText(u8"/wadm")
							imgui.NextColumn()
							...
							imgui.Separator()
							imgui.Columns(3)
							imgui.CenterColumnText(u8"/wfast")
							imgui.NextColumn()
			...
			if imgui.BeginTabItem(fa.EYE_LOW_VISION..u8' Админ-команды') then 
			 	imgui.BeginChild('##1', imgui.ImVec2(589 * MONET_DPI_SCALE, 330 * MONET_DPI_SCALE), true)
				--imgui.Columns(2)
				if imgui.SmallButton(u8'1 Административный уровень') then
					imgui.OpenPopup(fa.EYE_LOW_VISION .. u8' 1 Административный уровень')
				end
					if imgui.BeginPopupModal(fa.EYE_LOW_VISION .. u8' 1 Административный уровень', _, imgui.WindowFlags.NoCollapse  + imgui.WindowFlags.NoResize ) then
						imgui.Text(u8'/a(dmin) [TEXT] - чат администрации')
						imgui.Text(u8'/apanel - открыть панель администратора')
						imgui.Text(u8'/skin [ID] - изменить себе скин')
						imgui.Text(u8'/re(con) [ID] - начать слежку за игроком')
						imgui.Text(u8'/aleaders - посмотреть лидеров оффлайн')
						imgui.Text(u8'/tp - телепорт по специальным местам')
						imgui.Text(u8'/gotoderby - телепортироваться на дерби зону')
						imgui.Text(u8'/changekey - изменить пароль авторизации')
						imgui.Text(u8'/mlist - список замученных')
						imgui.Text(u8'/admins - список администрации')
						imgui.Text(u8'/pm [ID] [TEXT] - ответ игроку')
						imgui.Text(u8'/gg [ID] - пожелать хорошей игры')
						imgui.Text(u8'/ans [ID] [TEXT] - ответить с надписью после текста [Приятной игры]')
						imgui.Separator()
						...
						imgui.Separator()
				if imgui.SmallButton(u8'2 Административный уровень') then
					imgui.OpenPopup(fa.EYE_LOW_VISION .. u8' 2 Административный уровень')
				end
					if imgui.BeginPopupModal(fa.EYE_LOW_VISION .. u8' 2 Административный уровень', _, imgui.WindowFlags.NoCollapse  + imgui.WindowFlags.NoResize ) then
						imgui.Text(u8'/a(dmin) [TEXT] - чат администрации')
						imgui.Text(u8'/apanel - открыть панель администратора')
						imgui.Text(u8'/skin [ID] - изменить себе скин')
						imgui.Text(u8'/re(con) [ID] - начать слежку за игроком')
					...
					if imgui.BeginTabItem(fa.BARS..u8' Дополнительные функции') then 
						if imgui.BeginChild('##99', imgui.ImVec2(589 * MONET_DPI_SCALE, 333 * MONET_DPI_SCALE), true) then
							if isMonetLoader() then
								...
								if imgui.RadioButtonIntPtr(u8" Только используя команду /wfast [ID игрока]", fastmenu_type, 0) then
									fastmenu_type[0] = 0
									settings.general.mobile_fastmenu_button = false
									save_settings()
									FastMenuButton[0] = false
								end
								if imgui.RadioButtonIntPtr(u8' Используя команду /wfast [ID игрока] или кнопку "Взаимодействие" в левом углу экрана', fastmenu_type, 1) then
									fastmenu_type[0] = 1
								...
								if imgui.BeginPopupModal(fa.KEYBOARD .. u8' Настройка клавиш', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  ) then
									...
									if settings.general.use_binds and hotkey_no_errors then
										imgui.Separator()
										imgui.CenterText(u8'Открытие главного меню хелпера (аналог /wadm):')
										...
										imgui.Separator()
										imgui.CenterText(u8'Открытие быстрого меню взаимодействия с игроком (аналог /wfast):')
										...
										imgui.Separator()
										imgui.CenterText(u8'Приостановить команду (аналог /stop):')
										local width = imgui.GetWindowWidth()
				...
				if imgui.BeginChild('##smartuk', imgui.ImVec2(292 * MONET_DPI_SCALE, 340 * MONET_DPI_SCALE), true) then
					...
					if imgui.Button(fa.DOWNLOAD .. u8' Загрузить ##smartuk') then
						if getARZServerNumber() ~= 0 then
							...
							imgui.OpenPopup(fa.CIRCLE_INFO .. u8' wADM - Оповещение##donwloadsmartuk')
						else
							imgui.OpenPopup(fa.CIRCLE_INFO .. u8' wADM - Оповещение##nocloudsmartuk')
						end
					end
					if imgui.BeginPopupModal(fa.CIRCLE_INFO .. u8' wADM - Оповещение##nocloudsmartuk', _, imgui.WindowFlags.NoCollapse  + imgui.WindowFlags.NoResize) then
					...
					if imgui.BeginPopupModal(fa.CIRCLE_INFO .. u8' wADM - Оповещение##donwloadsmartuk', _, imgui.WindowFlags.NoCollapse  + imgui.WindowFlags.NoResize) then
											...
											if imgui.BeginPopupModal(fa.PEN_TO_SQUARE .. u8(" Редактирование подпункта##") .. chapter.name .. index .. chapter_index, _, imgui.WindowFlags.NoCollapse  + imgui.WindowFlags.NoResize) then
												...
												if imgui.Button(fa.FLOPPY_DISK .. u8' Сохранить', imgui.ImVec2(imgui.GetMiddleButtonX(2), 0)) then
													if u8:decode(ffi.string(input_smartuk_lvl)) ~= '' and not u8:decode(ffi.string(input_smartuk_lvl)):find('%D') and tonumber(u8:decode(ffi.string(input_smartuk_lvl))) >= 1 and tonumber(u8:decode(ffi.string(input_smartuk_lvl))) <= 6 and u8:decode(ffi.string(input_smartuk_text)) ~= '' and u8:decode(ffi.string(input_smartuk_reason)) ~= '' then
														...
														sampAddChatMessage('[wADM] {ffffff}Ошибка в указанных данных, исправьте!', message_color)
													end
												end
												imgui.EndPopup()
											end
				...
				if imgui.BeginChild('##smartpdd', imgui.ImVec2(292 * MONET_DPI_SCALE, 340 * MONET_DPI_SCALE), true) then
					...
					if imgui.Button(fa.DOWNLOAD .. u8' Загрузить ##smartpdd') then
						if getARZServerNumber() ~= 0 then
							...
							imgui.OpenPopup(fa.CIRCLE_INFO .. u8' wADM - Оповещение##donwloadsmartpdd')
						else
							imgui.OpenPopup(fa.CIRCLE_INFO .. u8' wADM - Оповещение##nocloudsmartpdd')
						end
					end
					if imgui.BeginPopupModal(fa.CIRCLE_INFO .. u8' wADM - Оповещение##nocloudsmartpdd', _, imgui.WindowFlags.NoCollapse  + imgui.WindowFlags.NoResize) then
					...
					if imgui.BeginPopupModal(fa.CIRCLE_INFO .. u8' wADM - Оповещение##donwloadsmartpdd', _, imgui.WindowFlags.NoCollapse  + imgui.WindowFlags.NoResize) then
			...
			--[[if imgui.BeginTabItem(fa.EYE..u8' Правила сервера') then 
			 	imgui.BeginChild('##1', imgui.ImVec2(589 * MONET_DPI_SCALE, 330 * MONET_DPI_SCALE), true)
				imgui.Columns(2)
				imgui.CenterColumnText(u8"№ правила")
				imgui.SetColumnWidth(-1, 295 * MONET_DPI_SCALE)
				imgui.NextColumn()
				imgui.CenterColumnText(u8"Наказание")
				imgui.SetColumnWidth(-1, 150 * MONET_DPI_SCALE)
				imgui.Columns(1)
				imgui.Separator()
				imgui.EndChild()
			...
			if imgui.BeginTabItem(fa.USER..u8' Настройки') then 
				...
				imgui.Separator()
				--imgui.Text(fa.CIRCLE_USER..u8" Разработчик хелпера: howtomw [Wanie Axmenberg]")
				--imgui.Separator()
				imgui.Text(fa.CIRCLE_USER ..u8" Разработчик хелпера")
				imgui.SameLine()
				if imgui.SmallButton(u8'howtomw [Wanie Axmenberg]') then
					openLink('https://vk.me/howtomw')
				end
				--imgui.Text(fa.CIRCLE_USER..u8" Главный тестер: Arab [Desseluter Axmenberg]")
				imgui.Separator()
				imgui.Text(fa.CIRCLE_USER ..u8" Главный тестер")
				imgui.SameLine()
				if imgui.SmallButton(u8'Arab [Desseluter Axmenberg]') then
					openLink('https://vk.me/ara_b')
				end
				imgui.Separator()
				...
				if imgui.SmallButton(u8'Проверить обновления') then
					...
					if not result then
						sampAddChatMessage('[wADM] {ffffff}Произошла ошибка при попытке проверить наличие обновлений!', message_color)
					end
				end
				imgui.Separator()
				imgui.Separator()
				...
				--end
				--imgui.Separator()
				imgui.Text(fa.GLOBE..u8" Группа wADM [ВКонтакте]:")
				imgui.SameLine()
				if imgui.SmallButton(u8'/tools_howtomw') then
					openLink('https://vk.com/tools_howtomw')
				end
				imgui.Separator()
				imgui.Text(fa.HAND_HOLDING_DOLLAR .. u8" Поддержать разработчика:")
				imgui.SameLine()
				...
				if imgui.BeginPopupModal(fa.SACK_DOLLAR .. u8' Поддержка разработчика', _, imgui.WindowFlags.NoCollapse  + imgui.WindowFlags.NoResize ) then
					--imgui.CenterText(u8'За реквизитами отпишите:')
				...
				imgui.CenterText(fa.PALETTE .. u8' Цветовое оформление хелпера:')
				imgui.Separator()
				...
				imgui.BeginChild("##2",imgui.ImVec2(589 * MONET_DPI_SCALE, 55 * MONET_DPI_SCALE),true)
				 imgui.TextWrapped(u8('Нашли баг или есть предложение по улучшению хелпера? Сообщите об этом мне в личные сообщения!'))
				 imgui.Separator()
				 imgui.TextWrapped(u8('Есть желание Вы можете закинуть донатик! Реквизиты есть выше)'))
				--imgui.CenterText(u8'Нашли баг или есть предложение по улучшению хелпера?')
				--imgui.Separator()
				--imgui.CenterText(u8'Сообщите об этом мне в личные сообщения!')
				imgui.EndChild()
				...
				if imgui.BeginPopupModal(fa.TRIANGLE_EXCLAMATION .. u8' Предупреждение ##off', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar ) then
					...
					if imgui.Button(fa.POWER_OFF .. u8' Да, выгрузить', imgui.ImVec2(200 * MONET_DPI_SCALE, 25 * MONET_DPI_SCALE)) then
						reload_script = true
						play_error_sound()
						sampAddChatMessage('[wADM] {ffffff}Хелпер приостановил свою работу до следущего входа в игру!', message_color)
						if not isMonetLoader() then 
							sampAddChatMessage('[wADM] {ffffff}Либо используйте ' .. message_color_hex .. 'CTRL {ffffff}+ ' .. message_color_hex .. 'R {ffffff}чтобы запустить хелпер.', message_color)
						end
						thisScript():unload()
					end
					imgui.End()
				...
				if imgui.BeginPopupModal(fa.TRIANGLE_EXCLAMATION .. u8' Предупреждение ##delete', _, imgui.WindowFlags.NoResize ) then
					imgui.CenterText(u8'Вы действительно хотите удалить wADM?')
					imgui.Separator()
					if imgui.Button(fa.CIRCLE_XMARK .. u8' Нет, отменить', imgui.ImVec2(200 * MONET_DPI_SCALE, 25 * MONET_DPI_SCALE)) then
						imgui.CloseCurrentPopup()
					end
					...
					if imgui.Button(fa.TRASH_CAN .. u8' Да, я хочу удалить', imgui.ImVec2(200 * MONET_DPI_SCALE, 25 * MONET_DPI_SCALE)) then
						sampAddChatMessage('[wADM] {ffffff}Хелпер полностю удалён с вашего устройства!', message_color)
						sampShowDialog(999999, message_color_hex .. "wADM", "Мне очень жаль что вы удалили wADM со своего устройства.\nЕсли удаление связано с негативным опытом использования, или вы столкнулись с багами или проблемами, то\nсообщите мне что именно заставило вас удалить хелпер: vk.me/howtomw\nЕсли что, вы можете заново скачать и установить хелпер в любой момент :)", "Закрыть", '', 0)
						reload_script = true
						play_error_sound()
...
imgui.OnFrame(
    ...
    function(player)
		...
		imgui.Begin(fa.BUILDING_SHIELD .. " wADM##fast_meg_button", MegafonWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoTitleBar  + imgui.WindowFlags.NoBackground )
			...
			if ffi.string(input_cmd):find('%W') or ffi.string(input_cmd) == '' or ffi.string(input_description) == '' or ffi.string(input_text) == '' then
				...
				for _, command in ipairs(temp_array) do
					if command.cmd == change_cmd and command.description == change_description and command.arg == change_arg and command.text:gsub('&', '\n') == change_text then
						...
						save_commands()
						if command.arg == '' then
							sampAddChatMessage('[wADM] {ffffff}Команда ' .. message_color_hex .. '/' .. new_command .. ' {ffffff}успешно сохранена!', message_color)
						elseif command.arg == '{arg}' then
							sampAddChatMessage('[wADM] {ffffff}Команда ' .. message_color_hex .. '/' .. new_command .. ' [аргумент] {ffffff}успешно сохранена!', message_color)
						elseif command.arg == '{arg_id}' then
							sampAddChatMessage('[wADM] {ffffff}Команда ' .. message_color_hex .. '/' .. new_command .. ' [ID игрока] {ffffff}успешно сохранена!', message_color)
						elseif command.arg == '{arg_id} {arg2}' then
							sampAddChatMessage('[wADM] {ffffff}Команда ' .. message_color_hex .. '/' .. new_command .. ' [ID игрока] [аргумент] {ffffff}успешно сохранена!', message_color)
						elseif command.arg == '{arg_id} {arg2} {arg3}' then
							sampAddChatMessage('[wADM] {ffffff}Команда ' .. message_color_hex .. '/' .. new_command .. ' [ID игрока] [аргумент] [аргумент] {ffffff}успешно сохранена!', message_color)
						end
...
imgui.OnFrame(
    ...
    function(player)
		imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(fa.BUILDING_SHIELD.." wADM##rank", GiveRankMenu, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize)
		...
		if imgui.Button(fa.USER_NURSE .. u8" Выдать ранг" , imgui.ImVec2(imgui.GetMiddleButtonX(1), 0)) then
					...
					lua_thread.create(function()
						...
						for _, line in ipairs(lines) do 
							if command_stop then 
								...
								sampAddChatMessage('[wADM] {ffffff}Отыгровка команды /' .. command.cmd .. " успешно остановлена!", message_color) 
								return 
							end
							if wait_tag then
								for tag, replacement in pairs(tagReplacements) do
								...
								sampSendChat(line)
								if debug_mode then sampAddChatMessage('[wADM DEBUG] Отправляю сообщение: ' .. line, message_color) end
			...
			if not command_find then
				sampAddChatMessage('[wADM] {ffffff}Бинд для изменения ранга отсутствует либо отключён!', message_color)
				sampAddChatMessage('[wADM] {ffffff}Попробуйте сбросить настройки хелпера!', message_color)
...
imgui.OnFrame(
    ...
    function(player)
		...
		imgui.Begin(fa.BUILDING_SHIELD .. " wADM##CommandStopWindow", _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize )
...
imgui.OnFrame(
    ...
    function(player)
		imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY - 50 * MONET_DPI_SCALE), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(fa.BUILDING_SHIELD.." wADM##CommandPauseWindow", _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize )
...
imgui.OnFrame(
    ...
    function(player)
		...
		imgui.Begin(fa.HOSPITAL.." wADM##fast_menu_button", FastMenuButton, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoTitleBar  )
...
imgui.OnFrame(
    ...
    function(player)
		...
		imgui.Begin(fa.CLOUD .. u8" wADM ##info_menu", _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.AlwaysAutoResize )
		if not isMonetLoader() and not sampIsChatInputActive() then player.HideCursor = true else player.HideCursor = false end
		...
		--imgui.Text(fa.DATABASE .. u8(' Текущая дата: ') .. u8(tagReplacements.get_time()))
...
imgui.OnFrame(
    ...
    function(player)
		...
		imgui.SetNextWindowSize(imgui.ImVec2(225 * MONET_DPI_SCALE, 113 * MONET_DPI_SCALE), imgui.Cond.FirstUseEver)
		imgui.Begin(fa.BUILDING_SHIELD .. u8" wADM##patrool_info_menu", PatroolMenu, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.AlwaysAutoResize )
		if not isMonetLoader() and not sampIsChatInputActive() and not sampIsDialogActive() and not isSampfuncsConsoleActive() then player.HideCursor = true else player.HideCursor = false end
		if patrool_active then
			...
			if imgui.SmallButton(fa.GEAR) then
				imgui.OpenPopup(fa.BUILDING_SHIELD .. u8(' wADM##patrool_select_code'))
			end
			imgui.Separator()
			if imgui.Button(fa.CIRCLE_INFO .. u8(' Доклад'), imgui.ImVec2(100 * MONET_DPI_SCALE, 25 * MONET_DPI_SCALE)) then
				lua_thread.create(function ()
					if debug_mode then
						sampAddChatMessage('[wADM DEBUG] {ffffff}' .. '/r ' .. tagReplacements.my_doklad_nick() .. ' на CONTROL. Продолжаю патруль, нахожусь в районе ' .. tagReplacements.get_area() .. " (" .. tagReplacements.get_square() .. ').', message_color)
					end
					...
					if tagReplacements.get_car_units() ~= 'Нету' then
						if debug_mode then
							sampAddChatMessage('[wADM DEBUG] {ffffff}' .. '/r Патрулирую уже ' .. format_patrool_time(patrool_time) .. ' в составе юнита ' .. tagReplacements.get_car_units() .. ', состояние ' .. u8(tagReplacements.get_patrool_code()) .. '.', message_color)
						end
						...
						if debug_mode then
							sampAddChatMessage('[wADM DEBUG] {ffffff}' .. '/r Патрулирую уже ' .. format_patrool_time(patrool_time) .. ', состояние ' .. u8(tagReplacements.get_patrool_code()) .. '.', message_color)
						end
			...
			if imgui.Button(fa.CIRCLE_STOP .. u8(' Завершить'), imgui.ImVec2(100 * MONET_DPI_SCALE, 25 * MONET_DPI_SCALE)) then
				lua_thread.create(function ()
					...
					if debug_mode then
						sampAddChatMessage('[wADM DEBUG] {ffffff}' .. '/r ' .. tagReplacements.my_doklad_nick() .. ' на CONTROL. Завершаю патруль, освобождаю маркировку ' .. tagReplacements.get_patrool_mark() .. ', состояние ' .. tagReplacements.get_patrool_code(), message_color)
					end
					...
					wait(1200)
					if debug_mode then
						sampAddChatMessage('[wADM DEBUG] {ffffff}' .. '/r Патрулировал ' .. format_patrool_time(patrool_time), message_color)
					end
					...
					wait(1200)
					if debug_mode then
						sampAddChatMessage('[wADM DEBUG] {ffffff}' .. '/delvdesc', message_color)
					end
			...
			if imgui.Button(fa.CIRCLE_PLAY .. u8(' Начать патруль'), imgui.ImVec2(200 * MONET_DPI_SCALE, 25 * MONET_DPI_SCALE)) then
				if isCharInAnyCar(PLAYER_PED) then
					imgui.OpenPopup(fa.BUILDING_SHIELD .. u8(' wADM##start_patrool'))
				else
					PatroolMenu[0] = false
					sampAddChatMessage('[wADM] {ffffff}Нельзя начать патруль, вы должны быть за рулём транспорта!', message_color)
				end
			end
		end
		if imgui.BeginPopupModal(fa.BUILDING_SHIELD .. u8(' wADM##start_patrool'), _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  ) then
			player.HideCursor = false 
			...
			if imgui.Button(fa.PLAY .. u8' Начать патруль', imgui.ImVec2(150 * MONET_DPI_SCALE, 25 * MONET_DPI_SCALE)) then
				...
				lua_thread.create(function ()
					if debug_mode then
						sampAddChatMessage('[wADM DEBUG] {ffffff}' .. '/r ' .. tagReplacements.my_doklad_nick() .. ' на CONTROL. Начинаю патруль,  нахожусь в районе ' .. tagReplacements.get_area() .. " (" .. tagReplacements.get_square() .. ').', message_color)
					end
					...
					if tagReplacements.get_car_units() ~= 'Нету' then
						if debug_mode then
							sampAddChatMessage('[wADM DEBUG] {ffffff}' .. '/r Занимаю маркировку ' .. tagReplacements.get_patrool_mark() .. ', нахожусь в составе юнита ' .. tagReplacements.get_car_units() .. ', состояние ' .. tagReplacements.get_patrool_code() .. '.', message_color)
						end
						...
						if debug_mode then
							sampAddChatMessage('[wADM DEBUG] {ffffff}' .. '/r Занимаю маркировку ' .. tagReplacements.get_patrool_mark() .. ', состояние ' .. tagReplacements.get_patrool_code() .. '.', message_color)
						end
						sampSendChat('/r Занимаю маркировку ' .. tagReplacements.get_patrool_mark() .. ', состояние ' .. tagReplacements.get_patrool_code() .. '.')
					end
					wait(1200)
					if debug_mode then
						sampAddChatMessage('[wADM DEBUG] {ffffff}' .. '/vdesc ' .. tagReplacements.get_patrool_mark(), message_color)
					end
		...
		if imgui.BeginPopup(fa.BUILDING_SHIELD .. u8(' wADM##patrool_select_code'), _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  ) then
			player.HideCursor = false 
			imgui.PushItemWidth(150 * MONET_DPI_SCALE)
			if imgui.Combo('##patrool_code', ComboPatroolCode, ImItemsPatroolCode, #combo_patrool_code_list) then
				patrool_code = combo_patrool_code_list[ComboPatroolCode[0] + 1]
...
imgui.OnFrame(
    ...
    function(player)
		if player_id ~= nil and isParamSampID(player_id) then
			...
			sampAddChatMessage('[wADM] {ffffff}Прозиошла ошибка, ID игрока недействителен!', message_color)
			SobesMenu[0] = false
		end
    end
)
...
imgui.OnFrame(
    ...
    function(player)
        ...
        if smart_uk ~= nil and isParamSampID(player_id) then
            ...
            sampAddChatMessage('[wADM] {ffffff}Произошла ошибка умного розыска (нету данных либо игрок офнулся)!', message_color)
            SumMenuWindow[0] = false
        end
        imgui.End()
    end
...
imgui.OnFrame(
    ...
    function(player)
        ...
        if smart_pdd ~= nil and isParamSampID(player_id) then
            ...
            sampAddChatMessage('[wADM] {ffffff}Произошла ошибка умных штрафов (нету данных либо игрок офнулся)!', message_color)
            TsmMenuWindow[0] = false
        end
        imgui.End()
    end
...
function main()
	...
	if settings.player_info.name_surname == '' or settings.player_info.fraction == 'Неизвестно' then
		--sampAddChatMessage('[wADM] {ffffff}Пытаюсь получить ваш /stats поскольку остуствуют данные про вас!', message_color)
		check_stats = true
		--sampSendChat('/stats')
	end
	if settings.general.use_info_menu then
	...
	while true do
			...
			if isCharInAnyCar(PLAYER_PED) and settings.general.auto_change_code_siren then
				...
				if currentSirenState ~= lastSirenState then
					...
					if currentSirenState then
						sampAddChatMessage("[wADM] {ffffff}В вашем т/с была включена сирена, изменяю ситуационный код на CODE 3!", message_color)
						ComboPatroolCode[0] = 4
						patrool_code = combo_patrool_code_list[ComboPatroolCode[0] + 1]
					else
						sampAddChatMessage("[wADM] {ffffff}В вашем т/с была отключена сирена, изменяю ситуационный код на CODE 4.", message_color)
						ComboPatroolCode[0] = 5
						patrool_code = combo_patrool_code_list[ComboPatroolCode[0] + 1]
					end
				end
		...
		if ((os.date("%M", os.time()) == "55" and os.date("%S", os.time()) == "00") or (os.date("%M", os.time()) == "25" and os.date("%S", os.time()) == "00")) and settings.general.auto_notify_payday and getARZServerNumber() ~= '200' then
			sampAddChatMessage('[wADM] {ffffff}Через 5 минут будет PAYDAY.', message_color)
			wait(1000)
		end
...
function onScriptTerminate(script, game_quit)
    if script == thisScript() and not game_quit and not reload_script then
		sampAddChatMessage('[wADM] {ffffff}Произошла неизвестная ошибка, хелпер приостановил свою работу!', message_color)
		if not isMonetLoader() then 
			sampAddChatMessage('[wADM] {ffffff}Используйте ' .. message_color_hex .. 'CTRL {ffffff}+ ' .. message_color_hex .. 'R {ffffff}чтобы перезапустить хелпер.', message_color)
		end
